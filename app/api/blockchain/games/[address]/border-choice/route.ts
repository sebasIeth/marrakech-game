...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const data = encodeFunctionData({
      ...
      functionName: 'chooseBorderDirection',
      args: [dirUint8],
    });

    return NextResponse.json({
      transaction: { to: address, data, description: 'Choose Border Direction' },
    });
  } catch (error) {
    return NextResponse.json({ error: 'Failed to encode border choice' }, { status: 500 });
  }
}
